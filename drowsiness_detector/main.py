...
def main():
    ...
    # Snapshot the settings that produced this run. Thresholds move;
    # a session recorded without them stops being interpretable.
    logger = SessionLogger(
        enabled=SESSION_LOG,
        log_dir=SESSION_LOG_DIR,
        config_snapshot={
            "CNN_ENABLED": CNN_ENABLED,
            "CNN_FUSION_MODE": CNN_FUSION_MODE,
            "CNN_CLOSED_THRESHOLD": CNN_CLOSED_THRESHOLD,
            "CNN_EYE_MARGIN": CNN_EYE_MARGIN,
            "CNN_model_loaded": bool(eye_classifier.is_available()),
    ...
    try:
        while True:
            ...
            if calibrator.is_running():
                ...
                if result and result.is_valid():
                    ...
                    logger.event("calibrated",
                                 f"ear_th={ear_th} mar_th={mar_th} "
                                 f"yaw={baseline_yaw:+.1f} pitch={baseline_pitch:+.1f}")
            ...
            if not results.face_landmarks:
                logger.frame(face=0, phone=http_client.status()[0])
                draw_no_face(frame)
                _, jpeg = cv2.imencode(".jpg", raw_feed, [cv2.IMWRITE_JPEG_QUALITY, 55])
                streamer.push_jpeg(jpeg.tobytes())
                cv2.imshow("Drowsiness Detector — Q to quit, C to calibrate", frame)
            ...
            logger.frame(
                face=1, ear=ear, mar=mar, yaw=round(rel_yaw, 2),
                ...
                eyes_closed=int(eyes_closed), mouth_open=int(mouth_open),
                head_off=int(head_off), perclos=perclos,
                alerts="|".join(fired), phone=http_client.status()[0])
        ...
        # Teardown must run even if the loop raises or the user
        # hits Ctrl-C. The session log goes first: a crashed run is
        # exactly when its buffered rows matter most.
        logger.close()
        cap.release()
